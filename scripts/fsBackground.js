	  ...
	  function(request, sender, sendResponse) {
		...
		switch (request.message)
		{
			...
			case "checkHotkey": 
				...
				checkKey(cShortcutPref, cDefaultShortcut, function() {captureLastUsedMode();}) ||
		...
		chrome.tabs.getSelected(window.id,
			function(tab)
			{
				...
				try {
                    ...
                    var initMsg={topic: "init", mode: Mode};
                        ...
                        switch (msg.topic) {
                            case "initDone":
                                ...
                                switch (Mode) {
                                    ...
                                    case cModePC    :
                                        pluginCommand("captureInit");
                                        port.postMessage({topic: "selectPCArea"});
                                        break;
                                    case cModeMobile    :
                                        pluginCommand("captureInit");
                                        port.postMessage({topic: "selectMobileArea"});
                                        break;
                                    case cModeBrowser    :
                                        enableHotkey(false);
                                        lockItems();
...
function updateLastActionInContextMenu()
{
	...
	chrome.contextMenus.update(mnuEntireEdit, {title: "捕捉整个页面" + "...    " + getOption(cShortcutPrefEntire, cDefaultShortcutEntire)});
	chrome.contextMenus.update(mnuVisibleEdit, {title: "捕捉可见部分" + "...    " + getOption(cShortcutPrefVisible, cDefaultShortcutVisible)});
	chrome.contextMenus.update(mnuSelectedEdit, {title: "捕捉选定区域" + "...    " + getOption(cShortcutPrefSelection, cDefaultShortcutSelection)});
}
	...
	chrome.contextMenus.removeAll(
		function()
		{
			...
			if (!guiItemsLocked)
			{
				...
				if (fLite)
				{
					mnuEntireEdit = chrome.contextMenus.create({title: "捕捉整个页面" + "...", onclick: genericOnClick});
					mnuVisibleEdit = chrome.contextMenus.create({title: "捕捉可见部分" + "...", onclick: genericOnClick});
					mnuSelectedEdit = chrome.contextMenus.create({title: "捕捉选定区域" + "...", onclick: genericOnClick});
    ...
    iterateTabsAndCapture(tAction)
        .then(function(tabsCaptured){
            ...
            if (action === cActionMultiPDF) {
                ...
                pluginCommand("doGroupAction", {action: action.toString(), count: tabsCaptured.toString()});
        });
    /*
        .catch(function(e) {
            alert(e);
